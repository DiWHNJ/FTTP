...
def generate_images(
    ...
    if "SD" not in model_name:
        try:
            # model_path = (
            #     f'{model_name.replace("compvis","diffusers")}.pt'
            # )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", help="name of model", type=str, default="/data2/ljq/do_train/stable-diffusion/models/compvis-ga-method_full-alpha_0.1-epoch_20-lr_1e-05/diffusers-ga-method_full-alpha_0.1-epoch_20-lr_1e-05.pt", required=False)
    parser.add_argument(
        "--prompts_path", help="path to csv file with prompts", type=str, default="prompts/imagenette.csv", required=False
    )
    parser.add_argument(
        "--save_path", help="folder where to save images", type=str, default="evaluation_folder89/" ,required=False
    )
    parser.add_argument(
        "--device",
