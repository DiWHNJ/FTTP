...
class DDPM(pl.LightningModule):
    ...
    def get_loss(self, pred, target, mean=True,mask=None):
        ...
        # Apply mask for selective GA/GD
        if mask is not None:
            # pdb.set_trace()
            mask = mask.to(loss.device)
            mask = mask.unsqueeze(1)
            # For GA regions (mask == 1), use negative loss for gradient ascent
            ga_loss = loss * mask * -1 * 30
            
            # For GD regions (mask == 0), keep the original loss
            gd_loss = loss * (1 - mask) * 55
            
            # Combine GA and GD losses
            loss = ga_loss + gd_loss

        # Apply mean reduction if required
        if mean:
            loss = loss.mean()
        # pdb.set_trace()
        return loss
